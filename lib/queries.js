...
async function allInvoice(page, options = null) {
  ...
  const skip = (page - 1) * take;
  const takeOptions = {
    include: { amount: true, paymentDetails: true, nonPrintItems: true },
    take: take,
    skip: skip,
  };
...
async function cancelInvoice(invoiceId) {
  const [invoice, poData] = await Promise.all([
...
async function getJobData(page, options = null) {
  const take = 30;
  const skip = (page - 1) * take;
  const takeOptions = {
    take: take,
  ...
  const findOptions =
    options === null ? takeOptions : { ...options, ...takeOptions };
  const jobData = await prisma.purchaseOrder.findMany(findOptions);
  return jobData;
}

async function countJobData(options = null) {
  delete options.include;
  const jobDataCount = await prisma.purchaseOrder.count(options);
  return jobDataCount;
}

async function getPaymentData(page, options = null) {
  const take = 30;
  const skip = (page - 1) * take;
  const takeOptions = {
    include: {
  ...
  const payment = await prisma.paymentDetails.findMany(findOptions);
  return payment;
}
